...
func textField(
  ...
  styleMouseOver: TextFieldStyle = Theme.active.textFieldMouseOver
) -> TextFieldResult {
  var style = style
  if state.error {
    style = styleError
  ...
  scroll(
    ...
    if state.focused && Time.cursorSinBlinking >= 0 && !isTextSelected {
  ...
  let result = TextFieldResult(rect: textFieldBounds, changed: state.changed, hit: hit)
  state.changed = false
  return result
}
